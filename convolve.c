...
/*Create an "identity" impulse response. The output should be the sae as the nput when convolved with this*/
impulse_response[0] - 1.0;
impulse_size = 1;

/* Set the expect4ed size of the output signal */
...
/* Create a "delay" impulse response. The output should be delayed by 2 samples and be 1/2 the amplitude */
impulse_response[0] = 0.0;
impulse_response[1] = 0.0;
...
/* Do the convolution, and print the output signal */
convolve(input_signal, input_size, impulse_response, impulse_size, output_signal, output_size);
print_vector("Delayed 2 samples and 1/2 the amplitude", output_signal, output_size);


/* Create a "echo effect" The output will contain the original signal plus a copy delayed by 2 samples and 1/2 the amplitude. TH eoriginal and copy will overlap starting at the 3rd sample */
impulse_response[0] = 1.0;
impulse_response[1] = 0.0;
impulse_response[2] = 0.5;
impulse_size = 3;
...
/* Create a "echo effect" that doesn't overlap. The output will contain the original signal plus a copy delayed by 5 samples and 1/2 the amplitude */
impulse_response[0] = 1.0;
impulse_response[1] = 0.0;
...
/* Interchange the input signal and impulse response. Since convolution is comutative, you should get the same output */
...
void convolve(float x[], int N, float h[], int M, float y[], int P){
    ...
    int n, m;
    /* Make sure that output buffer is the right size: P = N + M - 1 */
    if(P != (N + M - 1)){
    ...
    /* Do the convolution */
    /* Outer loop: process x[n] with each sample of h[] */
    for(n = 0; n< N; n++){
        for(m = 0; m < M; m++){
            y[n+m] += x[n] * h[m];
        }
